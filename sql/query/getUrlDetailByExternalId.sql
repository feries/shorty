SELECT
    ...
    GROUP_CONCAT(DISTINCT b.`name`) AS "browser",
    GROUP_CONCAT(DISTINCT  o.`name`) AS "os"
FROM stat AS s
...
GROUP BY DATE_FORMAT(s.created_at, '%Y-%m-%d'), u.target_url, h.short_url, u.source_url;
